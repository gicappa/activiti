...
public class ProcessCancelledListenerDelegate implements ActivitiEventListener {
  ...
  private final List<ProcessRuntimeEventListener<ProcessCancelledEvent>> processRuntimeEventListeners;

  private final ToProcessCancelledConverter processCancelledConverter;
    ...
    if (event instanceof ActivitiProcessCancelledEvent) {
      processCancelledConverter.from((ActivitiProcessCancelledEvent) event)
        .ifPresent(convertedEvent -> {
          for (var listener : processRuntimeEventListeners) {
