...
public class ProcessInstanceEventsTest extends PluggableActivitiTestCase {
  ...
  public void testProcessInstanceEvents() throws Exception {
    ...
    repositoryService.suspendProcessDefinitionById(processInstance.getProcessDefinitionId(), true,
      null);
    repositoryService.activateProcessDefinitionById(processInstance.getProcessDefinitionId(), true,
      null);
    ...
    var processCancelledEvents = listener.filterEvents(
      ActivitiEventType.PROCESS_CANCELLED);
    assertThat(processCancelledEvents).hasSize(1);
    var cancelledEvent = (ActivitiCancelledEvent) processCancelledEvents.get(0);
  ...
  public void testSubProcessInstanceEvents() throws Exception {
    ...
    assertThat(processInstance).isNotNull();
    var processDefinitionId = processInstance.getProcessDefinitionId();
    ...
    event = (ActivitiEntityEvent) listener.getEventsReceived().get(1);
    var startChildExecutionId = event.getExecutionId();
    assertThat(event.getType()).isEqualTo(ActivitiEventType.ENTITY_CREATED);
    assertThat(event.getProcessInstanceId()).isEqualTo(processInstance.getId());
    ...
    event = (ActivitiEntityEvent) listener.getEventsReceived().get(3);
    var mainProcessExecutionId = event.getExecutionId();
    assertThat(event.getType()).isEqualTo(ActivitiEventType.ENTITY_INITIALIZED);
    assertThat(event.getProcessInstanceId()).isEqualTo(processInstance.getId());
    ...
    assertThat(event.getType()).isEqualTo(ActivitiEventType.ENTITY_CREATED);
    var subProcessEntity = (ExecutionEntity) event.getEntity();
    assertThat(subProcessEntity.getSuperExecutionId()).isEqualTo(startChildExecutionId);
    var subProcessInstanceId = subProcessEntity.getProcessInstanceId();
    ...
    var subProcessDefinitionId = ((ExecutionEntity) event.getEntity()).getProcessDefinitionId();
    assertThat(subProcessDefinitionId).isNotNull();
    var subProcessDefinition = repositoryService.getProcessDefinition(
  ...
  public void testProcessCompleted_NoEnd() {
    var noEndProcess = this.runtimeService.startProcessInstanceByKey("noEndProcess");
    var task = taskService.createTaskQuery().processInstanceId(noEndProcess.getId())
      .singleResult();
  ...
  @Deployment(resources = {"org/activiti/engine/test/api/runtime/oneTaskProcess.bpmn20.xml"})
  public void testProcessInstanceCancelledEvents_cancel() {
    ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("oneTaskProcess");
    assertThat(processInstance).isNotNull();
    listener.clearEventsReceived();
    ...
    ActivitiCancelledEvent processCancelledEvent = (ActivitiCancelledEvent) processCancelledEvents.get(
      0);
    assertThat(ActivitiCancelledEvent.class.isAssignableFrom(processCancelledEvent.getClass())).as(
    ...
    assertThat(taskCancelledEvents).as("ActivitiEventType.ACTIVITY_CANCELLED was expected 1 time.")
      .hasSize(1);
    ActivitiActivityCancelledEvent activityCancelledEvent = (ActivitiActivityCancelledEvent) taskCancelledEvents.get(
      0);
    assertThat(
      ActivitiActivityCancelledEvent.class.isAssignableFrom(activityCancelledEvent.getClass())).as(
      "The cause has to be the same as deleteProcessInstance method call").isTrue();
  ...
  public void testProcessInstanceCancelledEvents_cancelProcessHierarchy() throws Exception {
    ...
    assertThat(
      this.taskService.createTaskQuery().processInstanceId(processInstance.getId()).count()).as(
      "No task can be active for deleted process.").isEqualTo(0);
  ...
  @Deployment(resources = {"org/activiti/engine/test/api/runtime/oneTaskProcess.bpmn20.xml"})
  public void testProcessInstanceCancelledEvents_complete() {
    ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("oneTaskProcess");
    assertThat(processInstance).isNotNull();
  ...
  public void testProcessInstanceTerminatedEvents() {
    ...
    for (ActivitiEvent event : activityTerminatedEvents) {
      ...
      if (activityEvent.getActivityId().equals("theTask")) {
        ...
        assertThat(activityEvent.getActivityId()).as(
          "The user task must be terminated in the called sub process.").isEqualTo("theTask");
        assertThat(activityEvent.getCause()).as("The cause must be terminate end event")
  ...
  public void testDeleteMultiInstanceCallActivityProcessInstance() {
    ...
    assertThat(this.listener.getEventsReceived().get(0).getType()).as(
      "Task cancelled event has to be fired.").isEqualTo(ActivitiEventType.ACTIVITY_CANCELLED);
    assertThat(this.listener.getEventsReceived().get(2).getType()).as(
      "SubProcess cancelled event has to be fired.").isEqualTo(ActivitiEventType.PROCESS_CANCELLED);
