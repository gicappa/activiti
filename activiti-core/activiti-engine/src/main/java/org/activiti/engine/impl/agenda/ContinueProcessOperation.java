...
public class ContinueProcessOperation extends AbstractOperation {
  ...
  public void run() {
    ...
    if (currentFlowElement instanceof FlowNode) {
      ...
      throw new ActivitiException(
        "Programmatic error: no current flow element found or invalid type: " + currentFlowElement
          + ". Halting.");
    }
  }
  ...
  protected void executeProcessStartExecutionListeners() {
    ...
    executeExecutionListeners(process, execution.getParent(), EVENTNAME_START);
  }

  protected void continueThroughFlowNode(FlowNode flowNode) {

    // Check if it's the initial flow element. If so, we must fire
    // the execution listeners for the process too
    ...
    if (flowNode instanceof SubProcess && !isMultiInstance(flowNode)) {
  ...
  protected Boolean isMultiInstance(FlowNode flowNode) {
    return Optional.ofNullable(flowNode).filter(Activity.class::isInstance)
      .map(Activity.class::cast).map(Activity::hasMultiInstanceLoopCharacteristics).orElse(false);
  }
  ...
  protected void createChildExecutionForSubProcess(SubProcess subProcess) {
    ...
    var subProcessExecution = commandContext.getExecutionEntityManager()
  ...
  protected void executeSynchronous(FlowNode flowNode) {
    ...
    if (CollectionUtil.isNotEmpty(flowNode.getExecutionListeners())) {
      executeExecutionListeners(flowNode, EVENTNAME_START);
    }
    ...
    if (!inCompensation && flowNode instanceof Activity) {
      ...
      var boundaryEvents = ((Activity) flowNode).getBoundaryEvents();
  ...
  protected void executeAsynchronous(FlowNode flowNode) {
    var job = commandContext.getJobManager().createAsyncJob(execution, flowNode.isExclusive());
  ...
  protected void executeActivityBehavior(ActivityBehavior activityBehavior, FlowNode flowNode) {
    ...
    if (Context.getProcessEngineConfiguration() != null && Context.getProcessEngineConfiguration()
      .getEventDispatcher().isEnabled()
      && !(activityBehavior instanceof MultiInstanceActivityBehavior)) {
      Context.getProcessEngineConfiguration().getEventDispatcher().dispatchEvent(
        ActivitiEventBuilder.createActivityEvent(ActivitiEventType.ACTIVITY_STARTED, execution,
          flowNode));
    }
  ...
  protected void continueThroughSequenceFlow(SequenceFlow sequenceFlow) {
    ...
    if (CollectionUtil.isNotEmpty(sequenceFlow.getExecutionListeners())) {
      executeExecutionListeners(sequenceFlow, EVENTNAME_START);
    ...
    if (Context.getProcessEngineConfiguration() != null && Context.getProcessEngineConfiguration()
      .getEventDispatcher().isEnabled()) {
      var sourceFlowElement = sequenceFlow.getSourceFlowElement();
      var targetFlowElement = sequenceFlow.getTargetFlowElement();
      Context.getProcessEngineConfiguration().getEventDispatcher().dispatchEvent(
        ActivitiEventBuilder.createSequenceFlowTakenEvent(execution,
          SEQUENCEFLOW_TAKEN, sequenceFlow.getId(),
  ...
  protected void executeBoundaryEvents(
    ...
    for (BoundaryEvent boundaryEvent : boundaryEvents) {
      ...
      var childExecutionEntity =
        commandContext.getExecutionEntityManager().createChildExecution(execution);

      childExecutionEntity.setParentId(execution.getId());
      childExecutionEntity.setCurrentFlowElement(boundaryEvent);
      childExecutionEntity.setScope(false);

      var boundaryEventBehavior = ((ActivityBehavior) boundaryEvent.getBehavior());
