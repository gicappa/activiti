...
public class ProcessInstanceHelper {
  ...
  public Process getActiveProcess(ProcessDefinition processDefinition) {
    ...
    var process = ProcessDefinitionUtil.getProcess(processDefinition.getId());
    if (process == null) {
      throw new ActivitiException(
        "Cannot start process instance. Process model " + processDefinition.getName() + " (id = "
  ...
  public ProcessInstance createProcessInstance(ProcessDefinition processDefinition,
    ...
    return createProcessInstanceWithInitialFlowElement(processDefinition,
      businessKey,
      processInstanceName,
      initialFlowElement,
      process);
  ...
  public ProcessInstance createAndStartProcessInstanceByMessage(
    ...
    var bpmnModel = ProcessDefinitionUtil.getBpmnModel(processDefinition.getId());
    for (var flowElement : process.getFlowElements()) {
      if (flowElement instanceof StartEvent startEvent) {
        if (CollectionUtil.isNotEmpty(startEvent.getEventDefinitions())
          ...
          var messageRef = messageEventDefinition.getMessageRef();
    ...
    startProcessInstance(processInstance, commandContext, messageVariables, initialFlowElement,
      transientVariables);
  ...
  public ProcessInstance createAndStartProcessInstanceWithInitialFlowElement(
    ...
    var processInstance = createProcessInstanceWithInitialFlowElement(processDefinition,
      businessKey,
      processInstanceName,
      initialFlowElement,
      process);
  ...
  private void createProcessVariables(
    ...
    if (variables != null) {
      for (var varName : variables.keySet()) {
        processInstance.setVariable(varName, variables.get(varName));
      }
    }
    if (transientVariables != null) {
      for (var varName : transientVariables.keySet()) {
  ...
  public void startProcessInstance(ExecutionEntity processInstance, CommandContext commandContext,
    ...
    var messageEventSubscriptions = new LinkedList<MessageEventSubscriptionEntity>();
    for (var flowElement : process.getFlowElements()) {
      if (flowElement instanceof EventSubProcess eventSubProcess) {
        for (var subElement : eventSubProcess.getFlowElements()) {
          if (subElement instanceof StartEvent startEvent) {
            if (CollectionUtil.isNotEmpty(startEvent.getEventDefinitions())) {
              var eventDefinition = startEvent.getEventDefinitions().get(0);
              if (eventDefinition instanceof MessageEventDefinition messageEventDefinition) {
                var bpmnModel = ProcessDefinitionUtil.getBpmnModel(
                ...
                var messageExecution = commandContext.getExecutionEntityManager()
                  .createChildExecution(processInstance);
                messageExecution.setCurrentFlowElement(startEvent);
                messageExecution.setEventScope(true);

                var messageName = getMessageName(commandContext,
                  messageEventDefinition,
                  messageExecution);

                var subscription = commandContext.getEventSubscriptionEntityManager()
                  .insertMessageEvent(messageName,
                    messageExecution);
                var correlationKey = getCorrelationKey(commandContext,
                  messageEventDefinition,
                  messageExecution);
    ...
    var execution = processInstance.getExecutions()
      .getFirst(); // There will always be one child execution created
    ...
    if (Context.getProcessEngineConfiguration().getEventDispatcher().isEnabled()) {
      ...
      for (MessageEventSubscriptionEntity messageEventSubscription : messageEventSubscriptions) {
        commandContext.getProcessEngineConfiguration().getEventDispatcher()
          .dispatchEvent(
            ActivitiEventBuilder.createMessageWaitingEvent(messageEventSubscription.getExecution(),
  ...
  protected void dispatchStartMessageReceivedEvent(ExecutionEntity processInstance,
    ...
    if (Context.getProcessEngineConfiguration().getEventDispatcher().isEnabled()) {
      ...
      var execution = processInstance.getExecutions().getFirst();
      var eventDispatcher = Context.getProcessEngineConfiguration()
        .getEventDispatcher();
