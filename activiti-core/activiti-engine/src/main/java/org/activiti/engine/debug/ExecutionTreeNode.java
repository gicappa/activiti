...
public class ExecutionTreeNode implements Iterable<ExecutionTreeNode> {
  ...
  protected void internalToString(StringBuilder strb, String prefix, boolean isTail) {
    strb.append(prefix).append(isTail ? "└── " : "├── ").append(getExecutionEntity().getId())
      .append(" : ").append(getCurrentFlowElementId()).append(", parent id ")
      ...
      .append(getExecutionEntity().isEnded() ? " (ended)" : "").append(System.lineSeparator());
    if (children != null) {
