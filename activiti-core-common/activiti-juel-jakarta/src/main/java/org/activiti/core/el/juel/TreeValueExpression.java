...
public final class TreeValueExpression extends ValueExpression {
   ...
   *
   * @param context used to resolve properties (<code>base.property</code> and
   ...
   * expressions consisting of a simple identifier or a nonliteral prefix, followed by a sequence of
   * property operators (<code>.</code> or <code>[]</code>)
   */
  ...
  /**
   * Answer <code>true</code> if this is a deferred expression (containing sub-expressions starting
   * with <code>#{</code>)
   */
  ...
  /**
   * Expressions are compared using the concept of a <em>structural id</em>: variable and function
   * names are anonymized such that two expressions with same tree structure will also have the same
